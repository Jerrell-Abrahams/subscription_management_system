...
// a domain is verified - swap this for e.g. 'POS System <onboarding@yourdomain.com>'
// once one is set up.
const FROM = 'POS System <onboarding@resend.dev>';
...
async function sendOnboardingEmail({ email, password, fullName }) {
  const { error } = await resend.emails.send({
    from: FROM,
    to: email,
    subject: 'Your POS System account is ready',
    html: `
      <p>Hi ${fullName || 'there'},</p>
      <p>An account has been created for you on POS System. Here are your login credentials:</p>
